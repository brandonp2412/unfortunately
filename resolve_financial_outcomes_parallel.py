#!/usr/bin/env python3
"""Bounded-parallel runner for review-routed financial outcome resolution."""
...
from resolve_financial_outcomes import (
    MONEY_RE,
    direction,
    fetch_pdf,
    money_value,
...
def resolve_parallel(root: Path, workers: int) -> list[dict[str, str]]:
    ...
    review_indices = [
        ...
        if row["classified_outcome"] not in {"employee_win", "employer_win", "excluded"}
    ]
    print(f"Scoring {len(review_indices)} review-routed rows with {workers} workers", flush=True)

    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {pool.submit(score_row, root, source[i]): i for i in review_indices}
        done = 0
        for future in as_completed(futures):
            ...
            if done % 20 == 0 or done == len(review_indices):
                print(f"Scored {done}/{len(review_indices)} review-routed rows", flush=True)
    return result
...
def write_review_queues(root: Path, rows: list[dict[str, str]]) -> tuple[int, int, int]:
    ...
    for name, selected in (
        ("financial_review_queue.csv", flagged),
        ("financial_audit_high_risk.csv", high_risk),
        ("financial_outcome_changing_review.csv", outcome_changing),
    ):
...
def main() -> None:
    ...
    print(f"Resolved {len(resolved)} review-routed rows: {employee} employee wins, {employer} employer wins", flush=True)
