#!/usr/bin/env python3
"""Resolve mixed ERA merits outcomes with an observable-money tie-breaker.

Clear employee/employer legal outcomes are preserved.  Only ``mixed_unclear``
merits rows are converted to a binary outcome.  The rule is deliberately
mechanical: money ordered in the employee/applicant's favour minus money or
costs ordered against them.  A positive observable net is an employee win;
zero or negative is an employer win.

Private legal fees that do not appear in the determination are unknowable and
...
def main() -> None:
    ...
    neutral_money = sum(int(row["unallocated_money_units"] or 0) > 0 for row in mixed)
    print(f"Resolved {len(mixed)} mixed rows: {employee} employee wins, {employer} employer wins")
    print(f"Mixed rows with unallocated dollar-bearing order text: {neutral_money}")
