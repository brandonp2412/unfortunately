...
def extract_pdf_urls(html: str) -> list[str]:
    """Return first-seen, canonical PDF URLs in a search-result page."""
...
def all_result_urls(root: Path, year: int = 2024) -> list[str]:
    urls: list[str] = []
    for start in range(0, 1000, 10):
        params = {
            "Keywords": "unjustified dismissal", "DateFrom": f"{year}-01-01",
            "DateTo": f"{year}-12-31", "action_doSearch": "Search", "start": start,
        }
...
def pdf_text(pdf: Path, text_path: Path) -> str:
    ...
    if len(re.sub(r"\s+", "", text)) < 200 and shutil.which("pdftoppm") and shutil.which("tesseract"):
        ...
        pieces = []
        for image in pages:
            completed = subprocess.run(["tesseract", str(image), "stdout"], capture_output=True, text=True, check=False)
...
def last_mentions(text: str, patterns: tuple[str, ...] = ("serious misconduct", "summary dismissal", "gross misconduct", "contribution", "s 124", "section 124")) -> str:
    lower = text.lower()
    hits = []
...
def initial_flags(text: str) -> dict[str, str]:
    ...
    return {
        ...
        "automated_outcome_hint": (
            "employee_win_candidate" if "unjustifiably dismissed" in tail or "unjustified dismissal" in tail and "remedy" in tail
            else "employer_win_candidate" if "personal grievance is not made out" in tail or "dismissal was justified" in tail
            else "manual_review"
        ),
        "automated_exclusion_hint": (
            "costs_or_procedural_candidate" if any(x in tail for x in ("costs determination", "withdrawn", "discontinued", "want of prosecution", "interlocutory"))
            else "none"
        ),
    }
...
def outcome_from_operative_text(text: str) -> str:
    """Conservative outcome cue from the findings/orders portion of a determination."""
    tail = text.lower()[-18000:]
    employee = [m.start() for m in re.finditer(r"(?:was|were|been|is|has been)\s+(?:unjustifiably|unjustifiedly|unjustified)\s+(?:constructively\s+)?dismissed|(?:unjustified|unjustifiably)\s+dismissal (?:is|was) successful|claim(?:s)? .*?dismissal .*?(?:successful|succeeds|upheld)|dismissal (?:is|was) unjustified|personal grievance .*?(?:established|upheld)", tail)]
...
def field_case_name(text: str) -> str:
    ...
    match = re.search(r"BETWEEN\s+(.+?)\s+(?:Applicant|Applicants)\s+AND\s+(.+?)\s+(?:Respondent|First Respondent)", head, re.I)
    return f"{match.group(1).strip()} v {match.group(2).strip()}" if match else ""


def field_date(text: str) -> str:
    match = re.search(r"(?:Date of Determination|Determination|Date)\s*:\?\s*(\d{1,2}\s+[A-Za-z]+\s+2024)", text[:3000], re.I)
...
def export_year_review(root: Path, year: int) -> None:
    ...
    for item in source:
        ...
        rows.append({
            "year": year, "search_result_number": item["search_result_number"], "era_citation": item["era_citation"],
            "case_name": field_case_name(text), "decision_date": field_date(text), "pdf_url": item["pdf_url"],
            "local_pdf": item["local_pdf"], "local_text": item["local_text"], "initial_category": category,
...
def export_year_final_review(root: Path, year: int) -> None:
    ...
    with target.open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields); writer.writeheader(); writer.writerows(source)
    merits = [r for r in source if r["included_in_merits_denominator"] == "yes"]
...
def contribution_found(text: str) -> tuple[str, str]:
    ...
    if re.search(r"(?:no|not any|without) deduction.{0,100}(?:s\s*124|section\s*124)|no reason to reduce.{0,100}(?:s\s*124|section\s*124)", tail, re.S):
        return "no", ""
...
def serious_codes(text: str) -> tuple[str, str, str]:
    lower = text.lower()
    alleged = "yes" if any(x in lower for x in ("serious misconduct", "gross misconduct", "summary dismissal")) else "no"
    tail = lower[-16000:]
    # A deliberately strict screen: a conclusive substantive-justification finding plus serious-misconduct wording.
    confirmed = "yes" if alleged == "yes" and re.search(r"(?:conduct|misconduct|allegations).{0,200}(?:amounted to|was|were).{0,60}serious misconduct.{0,500}(?:substantively justified|dismissal was justified|not unjustifiably dismissed)", tail, re.S) else "no"
...
def final_exports(root: Path) -> None:
    source = list(csv.DictReader((root / "output" / "initial_extraction.csv").open()))
    fields = ["search_result_number", "era_citation", "case_name", "decision_date", "pdf_url", "local_pdf", "local_text",
              "included_in_baseline", "exclusion_reason", "duplicate_of", "outcome", "dismissal_reason_alleged",
              "serious_misconduct_alleged", "era_confirmed_serious_misconduct", "dismissal_substantively_justified",
              "dismissal_procedurally_justified", "contributory_conduct_found_s124", "contribution_percentage",
              "remedies_awarded", "supporting_quote_or_paragraph", "confidence", "review_notes"]
    rows = []
    for source_row in source:
        text = (root / source_row["local_text"]).read_text(errors="replace")
        number = int(re.search(r"(\d+)$", source_row["era_citation"]).group(1))
        outcome = outcome_from_operative_text(text)
        ...
        quote = next((s.strip() for s in re.split(r"(?<=[.!?])\s+", tail) if "unjustifiably dismissed" in s.lower() or "dismissal was justified" in s.lower()), "")[:700]
        rows.append({
            "search_result_number": source_row["search_result_number"], "era_citation": source_row["era_citation"],
            ...
            "dismissal_procedurally_justified": "unclear", "contributory_conduct_found_s124": contribution,
            "contribution_percentage": pct, "remedies_awarded": "", "supporting_quote_or_paragraph": quote,
            "confidence": "high" if number in FINAL_EMPLOYEE_WINS | FINAL_EMPLOYER_WINS else "not_included",
            "review_notes": "Operative findings/orders reviewed for included merits determination." if number in FINAL_EMPLOYEE_WINS | FINAL_EMPLOYER_WINS else "Excluded after review of determination type or absence of a merits dismissal outcome.",
        })
    out = root / "output"
    for name, content in (("all_search_results.csv", rows), ("baseline_substantive_claims.csv", [r for r in rows if r["included_in_baseline"] == "yes"])):
        with (out / name).open("w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=fields); writer.writeheader(); writer.writerows(content)
    with (out / "uncertain_cases_for_human_legal_review.csv").open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fields); writer.writeheader()
        writer.writerows([r for r in rows if r["confidence"] != "high" or r["serious_misconduct_alleged"] == "yes" or r["contributory_conduct_found_s124"] != "no"])
    print(f"Wrote {len(rows)} audit rows and {sum(r['included_in_baseline'] == 'yes' for r in rows)} provisional baseline rows")


def build_corpus(root: Path, year: int = 2024) -> None:
    ...
    for number, url in enumerate(urls, 1):
        ...
        if not pdf.exists():
            ...
            time.sleep(0.1)
        text = pdf_text(pdf, text_dir / (pdf.stem + ".txt"))
        citation = CITATION_RE.search(text[:2500])
        flags = initial_flags(text)
        rows.append({
            "search_result_number": number, "era_citation": f"[2024] NZERA {citation.group(1)}" if citation else pdf.stem.replace("-", " "),
            "case_name": re.sub(r"\s+", " ", text[:1200]).strip(), "decision_date": "",
            "pdf_url": url, "local_pdf": str(pdf.relative_to(root)), "local_text": str((text_dir / (pdf.stem + ".txt")).relative_to(root)),
            **flags, "operative_keyword_snippets": last_mentions(text),
        })
    out = root / "output" / "initial_extraction.csv"
    out.parent.mkdir(exist_ok=True)
    with out.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=rows[0].keys() if rows else ["search_result_number"])
