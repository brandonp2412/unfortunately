...
def horizontal_year_chart(
    ...
    for idx, key in enumerate(legend_items):
        ...
        x = left + col * col_w
        y = legend_y + row * 42
        draw.rounded_rectangle((x, y, x + 22, y + 22), radius=4, fill=COLORS.get(key, "#607D8B"))
    ...
    *,
    ...
    if many_legend_items:
        ...
        for idx, key in enumerate(legend_items):
            ...
            draw.text((x + 32, legend_y - 3), pretty(key), fill="#333333", font=LEGEND_FONT)
...
def line_chart(title: str, labels: list[str], series: dict[str, list[float]], path: Path) -> None:
    ...
    for idx, name in enumerate(series):
        ...
        draw.text((x + 40, legend_y - 3), pretty(name), fill="#333333", font=LEGEND_FONT)
...
def pie_chart(
    ...
    draw.ellipse((325, 300, 615, 590), fill="white")
    total_text = f"{total:,}"
    draw.text(
