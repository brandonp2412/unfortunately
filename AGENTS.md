...
12. Do not publish a partially classified corpus. `build_outcome_summaries.py` must fail if any included determination lacks either a legal-merits or monetary classification.

Automated outcome hints are routing aids only. Final legal classifications come from direct agent reading of the determination. Monetary classifications come from observable final orders plus direct audit where the parser cannot safely resolve them.
...
The final handoff includes the raw-results CSV, substantive-claims CSV, legal outcomes, monetary outcomes, disagreement/audit material, recall results, regenerated headline outputs/charts, and exact commands used.
